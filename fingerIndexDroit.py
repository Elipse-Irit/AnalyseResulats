		...
		for index, row in enumerate(fileColor) :
			if index > 0 :
				...
				if (row[6] == 'Red') :
					...
					if not first:
						draw.line((last[0], last[1], x, y), fill = (255, 0, 0), width=14)
					first = False
					last = (x,y)
		# Enregistre l'image
		if row[1] == 'ESC' :
			...
			contexte = '3'
		img.save("../Participants/Participant " + str(nameParticipant) + "/TraceParcours/" + row[0] + '_' + row[4] + '_' + row[3] + '_' + row[1] + '_Droit.png')
		#img.save("../Participants/Participant " + str(nameParticipant) + "/TraceParcours/IndexGauche.png")
